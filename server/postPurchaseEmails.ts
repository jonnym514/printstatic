/**
 * Post-Purchase Email Helper — extracted from marketing.ts for direct use by webhook
 *
 * The marketing router's triggerPostPurchaseSequence is a tRPC mutation, which is
 * awkward to call server-side from the webhook. This module exports the same logic
 * as a plain async function so the webhook can call it directly.
 *
 * Location on server: server/postPurchaseEmails.ts
 *
 * NOTE: If you prefer, you can instead use tRPC's createCallerFactory to call the
 * marketing router mutation directly. This standalone approach avoids coupling to
 * the tRPC context and is simpler for a webhook handler.
 */

import { enqueueEmail } from "./db";
import { logAgentAction } from "./db";
import { getProductById } from "../shared/products";

// Reuse the same email template builder from marketing.ts
// If buildEmailHtml is not exported, you'll need to export it or duplicate it here.
// For now we import it — make sure marketing.ts exports buildEmailHtml.
import { buildEmailHtml } from "./marketing";
